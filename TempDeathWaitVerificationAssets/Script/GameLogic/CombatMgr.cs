...
namespace Miner.GameLogic
{
    public class CombatMgr
    {
        ...
        private void BeginDeathWait()
        {
            ...
            ClearAgents();
            bool hasFutureAgents = HasFutureConfig(BaseConfig.GetLevelConfig(level), lastAgentIndex, gameTime);
            deathWaiting = true;
            if (mainView != null)
            {
        ...
        private void CompleteDeathWait()
        {
            ...
            if (!ShouldContinueAfterDeathWait(configs, lastAgentIndex, gameTime))
            {
                FinishLevel(false);
                return;
            }
